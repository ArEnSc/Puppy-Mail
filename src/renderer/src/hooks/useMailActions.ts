...
export function useMailActions() {
  ...
  const scheduleEmail = useCallback(async (scheduledEmail: ScheduledEmail) => {
    setIsLoading(true)
    try {
      const result = await window.electron.ipcRenderer.invoke('mailAction:scheduleEmail', scheduledEmail)
      return handleResult(result)
    } finally {
      setIsLoading(false)
    }
  ...
  const cancelScheduledEmail = useCallback(async (scheduledId: string) => {
    setIsLoading(true)
    try {
      const result = await window.electron.ipcRenderer.invoke('mailAction:cancelScheduledEmail', scheduledId)
      return handleResult(result)
    } finally {
      setIsLoading(false)
    }
  ...
  const updateDraft = useCallback(async (draftId: string, composition: Partial<EmailComposition>) => {
    setIsLoading(true)
    try {
      const result = await window.electron.ipcRenderer.invoke('mailAction:updateDraft', draftId, composition)
      return handleResult(result)
    } finally {
      setIsLoading(false)
    }
  ...
  const listenToInbox = useCallback(async (filter?: InboxListener['filter'], callback?: (email: EmailMessage) => void) => {
  ...
  const stopListening = useCallback(async (listenerId: string) => {
    setIsLoading(true)
    try {
      const result = await window.electron.ipcRenderer.invoke('mailAction:stopListening', listenerId)
  ...
  const checkForResponse = useCallback(async (originalMessageId: string) => {
    setIsLoading(true)
    try {
      const result = await window.electron.ipcRenderer.invoke('mailAction:checkForResponse', originalMessageId)
      return handleResult(result)
    } finally {
      setIsLoading(false)
    }
